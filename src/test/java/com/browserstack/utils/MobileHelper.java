...
public class MobileHelper {
  ...
  public boolean isAndroid() {
    return ((AppiumDriver) driver).getPlatformName()
      .equalsIgnoreCase("android");
  }

  public boolean isiOS() {
    return ((AppiumDriver) driver).getPlatformName().equalsIgnoreCase("ios");
  }

  public void selectFromPickerWheel(String iOSPicker, String option) {
    if (isAndroid()) {
      driver
        .findElement(MobileBy.xpath("//*[@text = '" + option + "']"))
        .click();
    } else if (isiOS()) {
      driver
        .findElement(By.xpath(iOSPicker))
        .sendKeys(new CharSequence[] { option });
  ...
  public void swipeLeft(WebElement el) {
    if (isAndroid()) {
      ...
      (new TouchAction(((AppiumDriver) driver))).waitAction(
          WaitOptions.waitOptions(Duration.ofMillis(500L))
        )
        .press(PointOption.point(dimension.getWidth() - 200, loc.getY()))
        .waitAction(WaitOptions.waitOptions(Duration.ofMillis(500L)))
        .moveTo(PointOption.point(dimension.getWidth() - 500, loc.getY()))
        .release()
        .perform();
    } else if (isiOS()) {
  ...
  public void scrollToElement(String accessibilityId) {
    if (isAndroid()) {
      ...
      RemoteWebElement element = (RemoteWebElement) driver.findElement(
        By.id(accessibilityId)
      );
